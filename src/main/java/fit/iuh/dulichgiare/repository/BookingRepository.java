...
public interface BookingRepository extends JpaRepository<Booking, Long> {
	...
	@Query("SELECT MONTH(b.createat), SUM(b.total), SUM(b.total - b.priceTour- b.priceVoucher) " + "FROM Booking b "
			+ "WHERE YEAR(b.createat) = :year " + "GROUP BY MONTH(b.createat)")
	List<Object[]> calculateRevenueByMonth(int year);

	List<Booking> findBookingByTourIdAndStatus(long id, String status);

	List<Booking> findBookingByCustomerIdAndStatusAndTourId(long id, String statusThanhCong, long tourId);
